...
exports.postSignup = (req, res, next) => {
  ...
  User.findOne({ email: req.body.email }, (err, existingUser) => {
    ...
    count: ['create', (results, done) => {
            let gid =1;
            const id = results.create._id;
            csyberUser.findById(id, function (err, count) {
                console.log(count)
                if(count.groups == undefined)gid = 0;
                else
                {
                    //count groups
                }
    ...
    addgroups: ['count', (results, done) => {
            const id = results.create._id;
            let gid = results.count;
            let group = {}
            group[gid++] = {name: "user"}
             var updateObj = {
                    $set:{
            ...
            console.log(results)
            csyberUser.findByIdAndUpdate(id, updateObj,{new:true},done);
       }
    ]
   }, (err, results) => {
